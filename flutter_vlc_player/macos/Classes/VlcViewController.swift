...
public class VLCViewController: NSObject {
    ...
    public func view() -> PlatformView {
        return hostedView
    }
    
    init(frame: PlatformRect, viewId: Int64, messenger:FlutterBinaryMessenger) {
        ...
        self.hostedView = PlatformView(frame: frame)
...
//        self.vlcMediaPlayer.libraryInstance.debugLoggingLevel = 3
    ...
    public func takeSnapshot() -> String? {
        ...
        #elseif os(macOS)
        guard let drawable = self.vlcMediaPlayer.drawable as? NSView else { return nil }
        let rep = drawable.bitmapImageRepForCachingDisplay(in: drawable.bounds) ?? NSBitmapImageRep(bitmapDataPlanes: nil, pixelsWide: Int(drawable.bounds.width), pixelsHigh: Int(drawable.bounds.height), bitsPerSample: 8, samplesPerPixel: 4, hasAlpha: true, isPlanar: false, colorSpaceName: .deviceRGB, bytesPerRow: 0, bitsPerPixel: 0)!
        drawable.cacheDisplay(in: drawable.bounds, to: rep)
        let image = NSImage(size: drawable.bounds.size)
